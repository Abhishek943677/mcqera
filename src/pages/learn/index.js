...
const Home = ({ lessonsObj }) => {
  ...
  return (
    <div>
      {JSON.parse(lessonsObj).map((item) => {
        return (
          <div className="p-2 rounded-md my-3 make-com-dark">
            <p className="text-center text-2xl py-1">{item.course}</p>
            <div>
              {item.subjectArray.map((subject) => {
                return (
                  <div className="mb-1 hover:opacity-50 cursor-pointer">
