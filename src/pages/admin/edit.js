...
export async function getServerSideProps(context) {
  ...
  // actual sever side stuffs

  try {
    const courseObj = await loadCourseObj(); //loads course object from mongoDB database

    var singleQuestion = {};
    if (context.query.id && context.query.subject && context.query.trade) {
      const res = await fetch(
        `${process.env.APP_URL}/api/question/editquestion?id=${context.query.id}&subject=${context.query.subject}&trade=${context.query.trade}`
      );
      ...
      //   if response is not as per assumption then it will redirect to home page
      if (!repo.ok) {
        return {
          redirect: {
            destination: "/",
    ...
    // if any error occurs then redirect back to home page
  } catch (error) {
    return {
      redirect: {
        destination: "/",
